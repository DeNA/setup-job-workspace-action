...
beforeEach(async () => {
  ...
  await fs.promises.writeFile(
    `${process.env.GITHUB_WORKSPACE!}/${dummyFile}`,
    dummyFileContent
  )
})
...
/**
 * Helper function to verify workspace replacement with repository name
 */
async function verifyWorkspaceWithRepository(
  ...
  const virtualWorkspacePath = path.join(
    runnerWorkParent,
    expectedRepoName,
    workspaceName || `${workflowName}-${jobName}`
  )
  ...
  expect(
    fs.lstatSync(`${process.env.GITHUB_WORKSPACE!}.bak`).isSymbolicLink()
  ).toBe(false)
  // /$GITHUB_WORKSPACE is symlink
  expect(fs.lstatSync(process.env.GITHUB_WORKSPACE!).isSymbolicLink()).toBe(
    true
  )
  ...
  expect(fs.readFileSync(virtualWorkspaceLinkFile, 'utf8')).toBe(
    dummyFileContent
  )
}

test('replaceWorkspace() with workspaceName', async () => {
  await verifyWorkspaceWithRepository(
    'my_repository',
    'test-dir',
    'my_repository'
  )
...
test('replaceWorkspace() with repository name and default input', async () => {
  await verifyWorkspaceWithRepository('my_repository', '', 'my_repository')
})
