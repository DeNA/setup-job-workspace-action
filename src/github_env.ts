...
export function getWorkflowName(): string {
  ...
  if (githubWorkflowRef != null) {
    ...
    const workflowYaml = githubWorkflowRef.match(
    ...
    core.debug(
      `Found GITHUB_WORKFLOW. workflow name: ${process.env.GITHUB_WORKFLOW}`
    )
    // GITHUB_WORKFLOW == ${{ github.workflow }} is `name` in yml: `name: 'build-test'`
    return process.env.GITHUB_WORKFLOW
  } else {
    throw new Error(
      'Both env GITHUB_WORKFLOW_REF and GITHUB_WORKFLOW are undefined!'
    )
  }
}
